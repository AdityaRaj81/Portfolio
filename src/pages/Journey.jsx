...
const Journey = () => {
  ...
  const journeyData = [
    {
      ...
      location: 'India',
      description: 'Working as an Associate, contributing to enterprise-level projects and leveraging modern technologies to deliver innovative solutions.',
  ...
  const stats = [
    ...
    { label: 'Companies', value: '3', icon: <Briefcase size={20} /> },
  ...
  const certifications = [
    {
      ...
      icon: <Zap size={20} />,
      items: [
        { name: 'Full Stack Web Development', issuer: 'Modern Academy', year: '2024', color: 'from-primary-500 to-accent-cyan' },
        { name: 'React - The Complete Guide', issuer: 'Udemy', year: '2023', color: 'from-accent-cyan to-primary-500' },
        { name: 'JavaScript Algorithms and Data Structures', issuer: 'freeCodeCamp', year: '2023', color: 'from-accent-purple to-accent-pink' }
      ]
    },
    {
      ...
      icon: <Target size={20} />,
      items: [
        { name: 'Problem Solving (Advanced)', issuer: 'HackerRank', year: '2024', color: 'from-accent-green to-green-600' },
        { name: 'Data Structures & Algorithms', issuer: 'Coding Practice', year: '2024', color: 'from-accent-purple to-primary-500' }
      ]
    }
  ];
    ...
    <div className="min-h-screen">
      ...
      <div className="min-h-screen p-4 lg:p-8 space-y-8 lg:space-y-12 pt-24 lg:pt-16">
        ...
        <div className="flex justify-center gap-4 flex-wrap">
          <button
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeItem === null
          ...
          <button
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeItem === 'current'
          ...
          <button
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeItem === 'education'
          ...
          <button
            className={`px-4 py-2 rounded-full text-sm font-medium transition-all duration-300 ${activeItem === 'work'
          ...
          <div className="space-y-6 lg:space-y-8">
            {journeyData.map((item, index) => {
              ...
              return (
                <div
                  key={index}
                  className={`relative timeline-item transition-all duration-700 ${isVisible && !isFiltered
                ...
                >
                  ...
                  <div className="absolute left-2 lg:left-6 w-4 h-4 bg-bg-secondary border-4 border-primary-500 rounded-full flex items-center justify-center z-10">
                    ...
                    <div className={`w-10 h-10 lg:w-12 lg:h-12 rounded-full bg-gradient-to-br ${item.gradient} flex items-center justify-center text-white shadow-xl -ml-3 -mt-3 lg:-ml-4 lg:-mt-4 transition-all duration-300 border-2 border-white/20 ${hoveredItem === index ? 'scale-110 shadow-2xl ring-4 ring-white/10' : ''
                  ...
                  <div className="ml-14 lg:ml-20">
                    <div className={`bg-bg-secondary/80 backdrop-blur-md rounded-xl lg:rounded-2xl p-4 lg:p-6 border border-border-primary transition-all duration-300 group relative overflow-hidden ${hoveredItem === index
                      ? 'border-accent-cyan shadow-glow-cyan -translate-y-2 scale-[1.02]'
                      : 'hover:border-accent-cyan/50 hover:shadow-glow'
                      }`}>

                      {/* Animated Background Gradient */}
                      <div className={`absolute inset-0 bg-gradient-to-br ${item.gradient} opacity-0 transition-opacity duration-500 ${hoveredItem === index ? 'opacity-5' : ''
                        }`}></div>
            ...
            {certifications.map((category, catIndex) => (
              <div key={catIndex} className="bg-bg-secondary/80 backdrop-blur-md rounded-2xl p-6 border border-border-primary hover:border-accent-cyan transition-all duration-300">
                <div className="flex items-center gap-3 mb-4">
                  <div className="w-8 h-8 bg-gradient-to-r from-primary-500 to-accent-purple rounded-lg flex items-center justify-center text-white">
                    {category.icon}
                  </div>
